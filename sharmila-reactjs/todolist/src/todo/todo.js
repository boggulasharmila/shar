...
function Todo({item, todoItems, setTodoItems}) {
  ...
  return (
    <div>
      ...
      {item.edit ?(
      <div>
        <input value = {item.todoItem} type="text" id='s' onChange={(event) => handleEditValue(event,item.id)} />
        <button id='save' onClick={() => editTodo(item.id)}>save</button>
        </div>
      ):(
        <div>
          <span 
          ...
          <button id='edit' onClick={() => editTodo(item.id)}>Edit</button>
          <button id='cancel'
            onClick={() => {
              deleteTodo(item.id);
            }}
